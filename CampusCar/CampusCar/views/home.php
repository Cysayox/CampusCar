...
<body>
   ...
   <?php include __DIR__ . '/layout/header.php'; ?>
    ...
    <script>
        ...
        // --- 1. MÉMORISATION DU SENS DU TRAJET ---
        // On récupère la valeur envoyée en PHP dans l'URL (si elle existe)
        const sensActuel = "<?= htmlspecialchars($_GET['sens_trajet'] ?? 'vers campus') ?>";
        
        // Si la page s'est chargée suite à une recherche "depuis campus", 
        // on simule un clic magique sur le bouton pour inverser l'interface !
    ...
    // --- 2. AUTOCOMPLÉTION API GOUVERNEMENT ---
        ...
        inputAdresse.addEventListener('input', async function(e) {
            ...
            // On attend que l'utilisateur ait tapé au moins 3 caractères pour ne pas spammer l'API
            if (saisie.length >= 3) {
                try {
                    // On interroge l'API (Astuce : on met les coordonnées de Fort-de-France pour prioriser les Antilles !)
                    ...
                    // On remplit avec les nouvelles adresses exactes renvoyées par l'État
                    ...
                    if (data.features) {
                        const adressesAntilles = data.features.filter(lieu => {
                            ...
                            // On vérifie que le code postal existe et commence par 971 ou 972
