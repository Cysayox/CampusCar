...
<style>
    /* Surlignage léger SANS modifier le fond */
    .trip-card.is-driver-card {
        border-left: 6px solid darkslateblue !important;
    }
    
    .trip-card.is-passenger-card {
        border-left: 6px solid deepskyblue !important;
    }
</style>
    ...
    <div class="results-container" style="margin-top: 40px;"> <h2 style="text-align: center; margin-bottom: 20px; color: var(--bbc-fonce);">Trajets disponibles</h2>
        ...
        if (count($trajets) > 0): 
            foreach ($trajets as $t): 
                ...
                // LOGIQUE DE SURLIGNAGE (Bandes de couleurs UNIQUEMENT)
                $highlight_class = '';
                if (isset($_SESSION['user_id'])) {
                    if ($t['id_conducteur'] == $_SESSION['user_id']) {
                        $highlight_class = 'is-driver-card';
                    } else {
                        ...
                        if ($stmt_check->rowCount() > 0) {
                            $highlight_class = 'is-passenger-card';
                        }
                    }
                }
        ?>
        
            <a href="index.php?action=trajet_details&id=<?= $t['id_trajet'] ?>" class="trip-card <?= $highlight_class ?>">
